...
\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.5\textwidth]{exercise3-2-1.pdf}
	\caption{All the data (training, validation, and test) with the two predicted regressions using sets A and B as training sets.}
	\label{fig:3-2}
\end{figure}

Because there was no test / validation set given, I split the validation set into two - one became the validation set and one became the test set. To find the best regression, first choose an $M$ value using ridge regression with $\lambda = 0$ and iterating over all possible values of $M$ as there are data points. The $M$ that gives the smallest SSE for the training set, use that to test values of $\lambda$ that minimize the SSE on the validation set. The results of using set A as training vs. set B as training is given in Figure \ref{fig:3-2}. The SSE for set A on the test set was $15.35$ and the SSE for set B on the test set was $0.77$. The reason that set B gives a less complex model is that the data used in the training set was not uniformly distributed around the optimal regression line - the errors from the training set were pulling the model coefficients into higher complexity.



In [15]: w0 = ridge_regression(x_train, y_train, m, l, np.mean(x_train, axis = 0), np.mean(y_train, axis = 0))
m =  0
lambda =  0
order 0
In [16]: w0
Out[16]: array([[ 6.75261634]])
SSE = 1372.20616144]
...
order 0
In [31]: w0
Out[31]: array([[ 6.75261634]])
SSE = 1372.20616144


singular matrix when m = 1 and lambda = 0.
